...
class LeRobotBridgeDataConfig(DataConfigFactory):
    """
    Bridge policy data config.
    """

    extra_delta_transform: bool = False
    # BridgeData uses "action" (singular) as the column name
    action_sequence_keys: Sequence[str] = ("action",)
                ...
                _transforms.RepackTransform(
                    {
                        ...
                        "state": "observation.state",
                        "actions": "action",
                        "prompt": "prompt",  # Preserve prompt added by PromptFromLeRobotTask
                    }
                )
            ]
        )
...
_CONFIGS = [
    ...
    *polaris_config.get_polaris_configs(),
    ...
    TrainConfig(                                                                              
      ...
      weight_loader=weight_loaders.CheckpointWeightLoader("gs://openpi-assets/checkpoints/pi05_base/params"),  # pi05 weights
